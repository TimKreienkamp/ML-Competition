...
results<- foreach(bucket = Llist, k = klist, .combine=rbind, .packages=c("class", "dplyr")) %dopar% {
  ...
  result <- c(bucket, k, testerror) #the result if the L eliminated points, the k neighbours and the errors made
}
colnames(results) <- c("testBucket", "k", "Error")
errorcrossvalidation<-as.data.frame(results) %>% group_by(k) %>% summarize(cvError=mean(Error))
